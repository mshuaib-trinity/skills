# Root Instructions Template — Agents Only

Use this template when the project should support generic agent runtimes only.
Generated files:

- `AGENTS.md`
- `.agents/`

Do not create `CLAUDE.md`, `.claude/`, or mirror rules in this mode.

## How to apply this template

**If `AGENTS.md` does NOT exist:**
Fill in every `{{PLACEHOLDER}}` and write the full template below to `AGENTS.md`.
...
```markdown
<!-- LIFECYCLE:BEGIN — generated by initialize-project-lifecycle; agents-only setup -->
...
| New durable rule or gotcha | `AGENTS.md` only if it belongs in the operating contract; otherwise the relevant docs file |
...
1. **No tracked changes before task tracking.** Durable artifacts and implementation for tracked work
   start only after a task exists in `tasks/current/`; operational and micro work are explicit exceptions.
2. **No parallel planning systems.** Designs, plans, grilling notes, and handoffs live in `tasks/`.
3. **No undocumented contract changes.** Code and docs move together.
4. **No secrets in code or logs.** Load secrets from env vars and redact sensitive data.
5. **Restricted files require explicit approval:** {{RESTRICTED_FILES}}
6. **Validation is mandatory.** `python3 scripts/validate-project.py` must pass before task closure.
