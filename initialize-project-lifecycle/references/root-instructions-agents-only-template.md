...
1. **No code before task tracking.** Meaningful work starts only after a task exists in `tasks/current/`.
...
6. **Validation is mandatory.** `python scripts/validate-project.py` must pass before task closure.
