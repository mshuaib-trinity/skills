...
description: Use when implementation and verification are done and the remaining work is task closure, branch integration, PR creation, merge, or cleanup.
---
...
**Announce at start:** "I'm using the finish-development-branch skill to complete this work."
...
### Step 1b: Close the Project Task (lifecycle protocol)

**If the repo has a task lifecycle** (root agent instructions plus `scripts/validate-project.py` — AgentKit does), close out the task this branch implements **before** integrating. Run the repo **Completion Checklist**:
...
- Run the project's completion checklist + `validate-project.py` before integrating (Step 1b, if the repo has a task lifecycle)
